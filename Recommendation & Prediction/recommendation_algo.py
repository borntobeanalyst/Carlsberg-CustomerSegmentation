...
#
#
# cat_var, num_var = test_for_cat(prediction_columns)
...
#https://www.instagram.com/p/Bz_g1rLl_k5/
my_class = DataCleanReco(prediction_columns_new_outlet, customers)
test = RecoSystemNewOutlet(my_class, 10, 5)


test.get_reco_db_new_outlet('CRL3000085248')


kings = "CRL3000085248"
customers.loc['CRL3000085248']['real_name']
my_next_class = RecoSystem(my_class, 10, 5)

l = ['keyword_1', 'keyword_2', 'keyword_3']
my_next_class.get_reco_db_v2('CRL3000085248')
my_next_class.get_similar_outlets_db('CRL3000085248')[l]
my_next_class.get_similar_outlets_db('CRL3000085248')['customer_city']
...
def get_reco_db_v2(self, crl):
    ...
    both_compared['optimal_combination'] = best_combination.index
    dict_2 = dict(zip(both_compared.recommended,both_compared.recommended_values))
    both_compared['optimal_values'] = both_compared['optimal_combination'].apply(lambda x: dict_2[x])
    both_compared['recommended_name'] = both_compared['recommended'].apply(lambda x: material.loc[x.split('_')[0]]['material'])
    return(both_compared)
