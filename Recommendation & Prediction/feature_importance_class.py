...
# all_columns = prediction_columns + ['total_consumption', 'avg_con_monthly']
#
# # test = [x for x in total_cols if x not in prediction_columns]
# # prediction_choices = ['total_consumption', 'avg_con_monthly']
# # what_to_predict = 'avg_con_monthly'
# #
# # customers_latest_cleaned[what_to_predict] = customers_latest[what_to_predict]
#
#
# customers_cleaned = customers[all_columns].dropna()
...
# X_train_cust, X_test_cust, y_train_cust, y_test_cust = DataClean_test.split_data()
ada_model_test = AdaModel(DataClean_test)
model = ada_model_test.model()
fig = ada_model_test.plot_feature_importance(model)

ada_model_test.feature_importances(model)

# optimized_model = ada_model_test.optimize_model()

# ada_model_test.evaluate_model(optimized_model)



ada_model_test.evaluate_model(model)
